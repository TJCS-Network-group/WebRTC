...
def check_and_login(stu_no, password=None):
    need = [
        Student.id, Student.stu_grade, Student.stu_password,
        Student.stu_enable, Student.stu_userlevel
    ]
...
def check_password_pattern(password: str) -> bool:
    ...
    result = [
        len(reps.findall(password))
        for reps in [num, small_letter, big_letter, special]
    ]
    # print(s,result)
...
def myinfo():
    ...
    return make_response_json(200,
                              "信息",
                              data={
                                  "stu_no": current_user.stu_no,
                                  "stu_name": current_user.stu_name
                              })
...
def change_password():
    ...
    current_user.stu_password = md5(passwd.encode("utf-8")).hexdigest()
    current_user.save()
    if current_user.stu_userlevel == User_level.Admin.value:
